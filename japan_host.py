#!/usr/bin/env python3
"""
Seedream Face Swap Generator - GitHub Hostable Version
Requires secrets to be configured via Streamlit Cloud or environment variables.
"""
...
def get_secret(key: str, default: str = None) -> str:
    ...
    Priority: st.secrets > environment variables > default
    """
...
# API Keys - Loaded from secrets/environment variables
REPLICATE_API_KEY = get_secret("REPLICATE_API_KEY", "")
ARK_API_KEY = get_secret("SEEDREAM_API_KEY", "")
COMFYUI_SERVER_URL = get_secret("COMFYUI_SERVER_URL", "")
...
class StreamlitImageGenerator:
    def __init__(self, replicate_api_key: str, seedream_api_key: str):
        self.replicate_api_key = replicate_api_key
        self.seedream_api_key = seedream_api_key
    ...
    def get_session(self):
        ...
        if not hasattr(self.local, 'session'):
            ...
            session.headers.update({
                "Authorization": f"Bearer {self.seedream_api_key}",
    ...
    def get_chatgpt_prompt(self, image_path):
    ...
    def process_single_image(self, image_path, selfie_path, enable_face_swap):
        ...
        try:
            ...
            prompt = self.get_chatgpt_prompt(str(image_path))
            if not prompt:
    ...
    def process_images(self, input_folder, selfie_file, enable_face_swap=False, comfyui_server_url=None):
...
def main():
    ...
    missing_secrets = check_secrets_configured()
    ...
    if missing_secrets:
        ...
        **For Streamlit Cloud:**
        ...
        COMFYUI_SERVER_URL = "http://your-comfyui-server/comfy"  # Optional
        ```
        ...
        **For local development:**
        ...
        export COMFYUI_SERVER_URL="http://your-comfyui-server/comfy"
        ```
        """)
        return
    ...
    # Check ComfyUI URL if face swap is enabled
    comfyui_server_url = COMFYUI_SERVER_URL if enable_face_swap else None

    if enable_face_swap and not COMFYUI_SERVER_URL:
        st.sidebar.warning("⚠️ COMFYUI_SERVER_URL not configured. Face swap will not work.")
    ...
    with col1:
        ...
        workflow_steps = [
            "**Configure secrets** (API keys) in Streamlit Cloud or environment",
    ...
    with col2:
        ...
        checks = []
        checks.append(("Replicate API Key", "✅" if REPLICATE_API_KEY else "❌"))
        checks.append(("Seedream API Key", "✅" if ARK_API_KEY else "❌"))
    ...
    can_process = REPLICATE_API_KEY and ARK_API_KEY and has_images and has_selfie

    if st.button("🚀 Start Parallel Processing", type="primary", disabled=not can_process):
        if not can_process:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if input_method == "Upload individual images":
                ...
                total_input_images = len(uploaded_images)
            else:
                # Extract ZIP file
                generator_temp = StreamlitImageGenerator(REPLICATE_API_KEY, ARK_API_KEY)
                extracted_files = generator_temp.extract_images_from_zip(uploaded_zip.getvalue(), temp_dir)
                total_input_images = len(extracted_files)
                if not extracted_files:
                    st.error("No valid images found in the ZIP file.")
                    return

            # Process images with parallel processing
            generator = StreamlitImageGenerator(REPLICATE_API_KEY, ARK_API_KEY)
            ...
            if results:
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as zip_file:
                    ...
                    summary += f"Success rate: {(len(results)/total_input_images*100):.1f}%\n"
                ...
                with col2:
                    st.metric("Success Rate", f"{(len(results)/total_input_images*100):.1f}%")
                ...
                if enable_face_swap:
                    process_info += f"\n- **ComfyUI Server**: Configured via secrets"
